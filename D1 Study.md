...
- GOOD 강의
  ...
  - Skill1(3도 나쁘진 않음)
  - 근접 콤보 스킬 - (Notify 부분 조금 알려줌.)
    - Notify : SendGameplayEventToActor를 통해 Actor한테 어떤 태그의 이벤트가 발생했는지(GameplayTag)를 전파하는 역할만 함. (종속성 방지)
  - 방패 막기와 캐릭터 선택창
    - 방패막기 관련 : UD1AttributeSet을 상속 받는 UD1CombatSet : 스텟 관련이겠지?
    - 처음에 캐릭터 선택창이 커지게 하는 것을 2개의 스킬로 관리하고 있음
      - 원래라면 하드코딩으로 게임모드라거나 그런 관련된 부분에서 우리가 처음에 게임을 시작했을 때 UI를 띄워라라는 코드는 넣는게 일반적인데 게임어빌리티 시스템을 통해 이를 처리.
  - 무기 교체와 물약 먹기 (Good) - effect 내용 나옴
    - AbilitySet_Hero 쪽에 인풋 태그를 연결 안해놨는데? - GA_ChangeEquip에 해놨음
...
- UGameplayMessageSubsystem
  ...
  - 언리얼 엔진에서 제공하는 메시지 시스템으로, 다양한 게임 시스템 간의 통신을 효율적으로 처리
  
  - 주요 특징
    
    1. 코드 종속성 감소
       - 시스템이나 컴포넌트 간 직접적인 참조 없이 통신 가능
       - 느슨한 결합(Loose Coupling) 구조 지원
       - 모듈 간 의존성 최소화
    2. 발행-구독 패턴(Publish-Subscribe)
       - 메시지를 발행하는 쪽은 누가 수신하는지 알 필요 없음
       - 관심 있는 리스너만 특정 메시지 타입을 구독하여 처리
       - 이벤트 기반 아키텍처 구현에 적합
    3. 시스템 간 커플링 감소
       - UI와 게임플레이 로직의 분리
       - 스킬 시스템과 다른 게임 시스템 간의 독립성 유지
       - 한 시스템의 변경이 다른 시스템에 미치는 영향 최소화
  
  - 델리게이트와의 비교
  
  | 특성                | UGameplayMessageSubsystem                   | 델리게이트(Delegate)                               |
  | ----------------- | ------------------------------------------- | --------------------------------------------- |
  | **결합도(Coupling)** | 느슨한 결합(Loose Coupling), 발신자와 수신자가 서로를 몰라도 됨 | 강한 결합(Tight Coupling), 발신자와 수신자가 서로를 직접 알아야 함 |
  | **범위**            | 전역적인 N:M 통신, 독립된 시스템 간 통신에 적합               | 주로 1:1 또는 1:N 통신, 긴밀히 연결된 시스템 간 통신에 적합        |
  | **식별 방식**         | 태그나 문자열 기반 채널을 통해 통신                        | 함수 포인터 기반, 직접적인 함수 호출                         |
  | **동적 구성**         | 런타임에 리스너 추가/제거 가능, 더 유연함                    | 코드에서 직접 연결 필요, 비교적 정적                         |
  | **데이터 전달**        | 구조체나 객체 형태로 데이터 전달 가능                       | 함수 시그니처에 정의된 매개변수만 전달 가능                      |
  | **디버깅**           | 전역 시스템으로 추적이 어려울 수 있음                       | 직접적인 호출로 추적이 비교적 쉬움                           |

- 게임 이펙트
  
  - 효과로 보면 안되고 해당 스킬을 사용했을 때 일어나는 변화라고 생각하는 게 조금 더 타당하다
  
  - ex. COST

- 링크 에러는 대부분의 경우 Build.cs에다가 모듈을 빼먹었을 때 발생한다.

![](D1%20Study_assets/2025-03-10-14-35-03-image.png)
